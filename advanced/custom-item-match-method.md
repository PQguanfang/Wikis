...
Items generated by [Supported Plugins](../base/directly-supported-plugins.md) will auto parse it's Item ID and compare it with the item ID you set in Item Format's `hook-item` option, so no matter how it changes, it will eventually sell normally.
